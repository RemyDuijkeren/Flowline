...
public class ValidationCacheTests : IDisposable
{
    ...
    public async Task EnsureDotNetAsync_UsesFreshCache()
    {
        ...
        await validator.EnsureDotNetAsync(new FlowlineRuntimeOptions(), noCache: false, CancellationToken.None);
        var result = await validator.EnsureDotNetAsync(new FlowlineRuntimeOptions(), noCache: false, CancellationToken.None);
    ...
    public async Task EnsureDotNetAsync_NoCacheRefreshesFreshCache()
    {
        ...
        await validator.EnsureDotNetAsync(new FlowlineRuntimeOptions(), noCache: false, CancellationToken.None);
        var result = await validator.EnsureDotNetAsync(new FlowlineRuntimeOptions(), noCache: true, CancellationToken.None);
    ...
    public async Task FailedChecks_AreNotPersisted()
    {
        ...
        Func<Task> act = () => validator.EnsureDotNetAsync(new FlowlineRuntimeOptions(), noCache: false, CancellationToken.None);
    ...
    public async Task EnvironmentAndSolutionKeys_AreNormalized()
    {
        ...
        await validator.GetEnvironmentInfoByUrlAsync("HTTPS://CONTOSO.CRM4.DYNAMICS.COM/", new FlowlineRuntimeOptions(), noCache: false, CancellationToken.None);
        await validator.GetEnvironmentInfoByUrlAsync("https://contoso.crm4.dynamics.com", new FlowlineRuntimeOptions(), noCache: false, CancellationToken.None);
        await validator.GetSolutionInfoAsync("HTTPS://CONTOSO.CRM4.DYNAMICS.COM/", "CONTOSOCORE", false, new FlowlineRuntimeOptions(), noCache: false, CancellationToken.None);
        await validator.GetSolutionInfoAsync("https://contoso.crm4.dynamics.com", "contosocore", false, new FlowlineRuntimeOptions(), noCache: false, CancellationToken.None);
    ...
    public async Task GetSolutionInfoAsync_BypassCache_IgnoresFreshCacheEntry()
    {
        ...
        await validator.GetSolutionInfoAsync("https://contoso.crm4.dynamics.com/", "ContosoCore", false, new FlowlineRuntimeOptions(), noCache: false, CancellationToken.None);
        var result = await validator.GetSolutionInfoAsync("https://contoso.crm4.dynamics.com/", "ContosoCore", false, new FlowlineRuntimeOptions(), noCache: false, CancellationToken.None, bypassCache: true);
