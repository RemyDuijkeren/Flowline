...
/// <summary>
/// KTD12: the inline surface warns that the next push will override the change it just made, and the
/// honest question is whether a push would actually act on the component — not whether the file mentions
/// it. An empty value is skipped by the apply tiers, so warning about one would train an operator to
/// ignore the warning.
/// </summary>
public class ConfigureApplyServiceWouldApplyTests
{
    ...
    public void AVariableWithAValue_WouldBeApplied() =>
        ConfigureApplyService.WouldApply(
    ...
    public void AConnectionReferenceWithAnEmptyConnectionId_WouldNotBeApplied() =>
        ConfigureApplyService.WouldApply(
                ...
                ConfigurableComponentKind.ConnectionReference, "contoso_Mailbox")
            .Should().BeFalse();
    ...
    public void ADeclaredFlowState_WouldBeApplied()
    {
        ...
        ConfigureApplyService.WouldApply(document, ConfigurableComponentKind.CloudFlow, "ApprovalFlow")
            .Should().BeTrue();
    }
    ...
    public void AComponentTheFileDoesNotName_WouldNotBeApplied()
    {
        ...
        ConfigureApplyService.WouldApply(document, ConfigurableComponentKind.CloudFlow, "SomeOtherFlow")
            .Should().BeFalse();
    }
    ...
    public void MatchingIsCaseInsensitive_LikeTheApplyPath()
    {
        ...
        ConfigureApplyService.WouldApply(document, ConfigurableComponentKind.Workflow, "CONTOSO_AUTONUMBER")
            .Should().BeTrue();
    }

    [Fact]
    public void AKindTheFileHasNoSectionFor_WouldNotBeApplied() =>
        ConfigureApplyService.WouldApply(new SettingsDocument(), ConfigurableComponentKind.PluginStep, "anything")
            .Should().BeFalse();
}
