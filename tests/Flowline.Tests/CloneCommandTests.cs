...
public class CloneCommandTests
{
    ...
    public void HasManagedContent_NoSrcFolder_ReturnsFalse()
    {
        ...
        try
        {
            ...
            var result = ProjectScaffolder.HasManagedContent(dataverseSolutionFolder);
    ...
    public void HasManagedContent_SrcWithOnlyUnmanagedFiles_ReturnsFalse()
    {
        ...
        try
        {
            ...
            var result = ProjectScaffolder.HasManagedContent(Path.Combine(root, "Solution"));
    ...
    public async Task AddDataverseSolutionProjectAsync_NoSolutionFile_WritesTheCdsprojEntryWithCSharpType()
    {
        ...
        try
        {
            var (created, added) = await ProjectScaffolder.AddDataverseSolutionProjectAsync(new MsBuildSolutionWriter(), root, slnPath, cdsprojPath);
    ...
    public async Task AddDataverseSolutionProjectAsync_NeverLeavesACsprojBesideTheCdsproj()
    {
        ...
        try
        {
            await ProjectScaffolder.AddDataverseSolutionProjectAsync(new MsBuildSolutionWriter(), root, slnPath, cdsprojPath);
    ...
    public async Task AddDataverseSolutionProjectAsync_EntryAlreadyPresent_DoesNotDuplicateIt()
    {
        ...
        try
        {
            ...
            await ProjectScaffolder.AddDataverseSolutionProjectAsync(writer, root, slnPath, cdsprojPath);

            var (created, added) = await ProjectScaffolder.AddDataverseSolutionProjectAsync(writer, root, slnPath, cdsprojPath);
    ...
    public void DescribeDataverseSolutionFolderWithoutCdsproj_StrayProjectFile_NamesItAndAsksForARename()
    {
        ...
        try
        {
            ...
            var message = ProjectScaffolder.DescribeDataverseSolutionFolderWithoutCdsproj(dataverseSolutionFolder, "CrO7982.cdsproj");
    ...
    {
        ProjectScaffolder.SolutionFileName("CrO7982").Should().Be("CrO7982.slnx");
    }
        ...
        string solutionName = "CrO7982", string? existingSolutionFileName = null)
    {
        ...
        var slnFilePath = ProjectScaffolder.ResolveSolutionFilePath(root, solutionName);

        var writer = new MsBuildSolutionWriter();
        await ProjectScaffolder.AddDataverseSolutionProjectAsync(writer, root, slnFilePath, cdsprojPath);

        foreach (var (folder, fileName) in new[]
                 {
                     ("Plugins", ProjectScaffolder.PluginsProjectFileName(solutionName)),
                     ("WebResources", ProjectScaffolder.WebResourcesProjectFileName(solutionName)),
                 })
        {
    ...
    public void PluginsProjectFileName_TakesTheSolutionNameVerbatim(string solutionName, string expected)
    {
        ...
        ProjectScaffolder.PluginsProjectFileName(solutionName).Should().Be(expected);
    }
    ...
    {
        ProjectScaffolder.WebResourcesProjectFileName(solutionName).Should().Be(expected);
    }
    ...
    {
        var message = ProjectScaffolder.DescribeCSharpKeywordCollision(solutionName);
    ...
    {
        ProjectScaffolder.DescribeCSharpKeywordCollision(solutionName).Should().BeNull();
    }
    ...
    public async Task PluginsProjectAlreadyRegistered_NothingAnywhere_ReturnsFalse()
    {
        ...
        try
        {
            ...
            var result = ProjectScaffolder.PluginsProjectAlreadyRegistered(Path.Combine(root, "Plugins"), layout);
    ...
    public async Task WebResourcesProjectAlreadyRegistered_NothingAnywhere_ReturnsFalse()
    {
        ...
        try
        {
            ...
            var result = ProjectScaffolder.WebResourcesProjectAlreadyRegistered(webresourcesCsproj, layout);
    ...
    public async Task WebResourcesProjectAlreadyRegistered_GenuineTieInSolutionFile_PropagatesConfigInvalid()
    {
        ...
        try
        {
            ...
            var act = () => ProjectScaffolder.WebResourcesProjectAlreadyRegistered(webresourcesCsproj, layout);
    ...
    public async Task ResolveExistingWebResourcesFolder_NothingAnywhere_ReturnsNull()
    {
        ...
        try
        {
            ...
            var result = ProjectScaffolder.ResolveExistingWebResourcesFolder(webresourcesFolder, webresourcesCsproj, layout);
    ...
    {
        var content = ProjectScaffolder.BuildAgentsFileContent(solutionName, $"{solutionName}.slnx", "Solution");
    ...
    public void BuildAgentsFileContent_FollowsARelocatedDataverseSolutionFolder()
    {
        ...
        var content = ProjectScaffolder.BuildAgentsFileContent("CrO7982", "CrO7982.slnx", "Dataverse");
    ...
    // gate that decides whether that branch runs at all; `PickSolutionAsync` is the branch's own logic,
    // exercised directly (like InitCommandTests exercises `CreateSolutionAsync`/`PickPublisherPrefixAsync`
    ...
    [Fact]
    public void ShouldPickSolution_NoSolutionNoRoleConfiguredInteractive_ReturnsTrue()
    {
        CloneCommand.ShouldPickSolution(new CloneCommand.Settings(), new ProjectConfig(), isInteractive: true).Should().BeTrue();
    }

    [Fact]
    public void ShouldPickSolution_SolutionNamed_ReturnsFalse()
    {
    ...
    public void ShouldPickSolution_AnyRoleUrlConfigured_ReturnsFalse(string roleProperty)
    {
        ...
        CloneCommand.ShouldPickSolution(new CloneCommand.Settings(), config, isInteractive: true).Should().BeFalse();
    }

    [Fact]
    public void ShouldPickSolution_NotInteractive_ReturnsFalse()
    {
        ...
        CloneCommand.ShouldPickSolution(new CloneCommand.Settings(), new ProjectConfig(), isInteractive: false).Should().BeFalse();
    }

    // ── PickSolutionAsync: R11 — unmanaged-only picker with a hidden-managed count ──

    [Fact]
    public async Task PickSolutionAsync_MixOfManagedAndUnmanaged_ListsOnlyUnmanagedAndReportsHiddenCount()
    {
    ...
    // ── PickSolutionAsync: the environment's Default solution is never offered ──

    [Fact]
    public async Task PickSolutionAsync_DefaultSolution_IsHiddenFromThePicker()
    {
        // 'Default' is the catch-all holding every unmanaged component in the environment — cloning it
        // is never the intent, so it's dropped (and it isn't counted as a hidden *managed* solution).
        var (command, console) = MakeCloneCommand();
        command.GetSolutionsOverride = (_, _) => Task.FromResult(new List<SolutionInfo>
        {
            MakeSolution("Default"),
            MakeSolution("CrO7982"),
        });
        console.Interactive();
        ...
        console.Output.Should().NotContain("Default");
        console.Output.Should().NotContain("managed solution hidden");
    }

    // ── PickSolutionAsync: nothing to adopt — stop and point at init, never offer to create here ──

    [Fact]
    public async Task PickSolutionAsync_NoUnmanagedSolutions_StopsAndPointsAtInit()
    {
        ...
        console.Output.Should().NotContain("Create new solution");
    }

    // ── PickSolutionAsync: R17 — a Sandbox source prompts the role, defaulting DEV ──

    [Fact]
    public async Task PickSolutionAsync_ExistingSolutionPickOnSandbox_PromptsRoleDefaultingDev()
    {
    ...
    // ── PickSolutionAsync: R17 — a Production source locks to the Prod role, no role prompt ──

    [Fact]
    public async Task PickSolutionAsync_ExistingSolutionPickOnProduction_LocksProdRole_NoRolePrompt()
    {
        var (command, console) = MakeCloneCommand("Production");
        command.GetSolutionsOverride = (_, _) => Task.FromResult(new List<SolutionInfo> { MakeSolution("CrO7982") });
