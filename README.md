...
- **Failed imports caught before they start** — Dataverse stops an import at the *first* component the target is missing, so you fix one, re-sync, re-import, and get told about the next. `deploy` now asks the target what it lacks up front and reports all of it in one pass, naming the solution that owns each component so you know whether to install it there or drop the dependency in DEV. Read-only, seconds, and it runs before the solution checker and the environment backup so a doomed deploy stops before the slow gates.
...
- **AI-native by design** — `sync` writes `DATAVERSE_CONTEXT.md` with your full schema (entities, attributes, option sets, forms, views, plugin steps); Copilot and Codex load it automatically via `AGENTS.md`; Claude Code via the `CLAUDE.md` file Flowline also scaffolds (a one-line `@AGENTS.md` import). Install Flowline as a [Claude Code / Codex plugin](https://github.com/RemyDuijkeren/Flowline/wiki/10-AI-Agents) (`/plugin marketplace add RemyDuijkeren/Flowline`) and an agent knows the full `clone → push → sync → deploy` loop — and how to migrate off other tools, like spkl — before you've even cloned a project.
