...
- **Failed imports caught before they start** — Dataverse stops an import at the *first* component the target is missing, so you fix one, re-sync, re-import, and get told about the next. `deploy` now asks the target what it lacks up front and reports all of it in one pass, naming the solution that owns each component so you know whether to install it there or drop the dependency in DEV. Read-only, seconds, and it runs before the backup so a doomed deploy costs nothing.
