...
├── docs/                              <-- Not scaffolded by clone; created on first `flowline sync`
...
A project still on the old layout fails closed with a clear `ConfigInvalid` error naming this section, rather than silently behaving as an empty or misconfigured project.
