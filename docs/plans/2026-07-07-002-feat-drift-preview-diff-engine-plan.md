...
- **KTD2 (revised post-review). `<target>` resolves a role keyword via `GetAndCheckEnvironmentInfoAsync`, or falls through to a raw URL via `FlowlineValidator.GetEnvironmentInfoByUrlAsync` when it isn't one of the four keywords — mirroring `DeployCommand`'s target-argument shape, not restricting to roles only.** The original decision here was role-only (config lookup + Prod-type safety check, no raw-URL support), on the reasoning that `GetAndCheckEnvironmentInfoAsync` is role-generic while `DeployCommand`'s own resolver additionally carries deploy-specific concerns (DTAP gate, managed-solution checks) that don't apply to a read-only comparison. That reasoning about *which resolver to reuse* still holds — `drift` still doesn't touch DTAP/managed-solution logic. But restricting `<target>` to role keywords only was a separate, avoidable choice: code review caught the description promising URL support the implementation didn't have (fixed at the time by narrowing the description, not the code), and a second pass found that the initial fix for *that* — four `--prod/--uat/--test/--dev` override options mirroring `CloneCommand` — didn't fit either, since `drift` only ever acts on one role per invocation while `CloneCommand`'s four flags exist because clone can configure all four roles in one run. The actual right shape was in `DeployCommand` the whole time: `<target>` itself accepts a role keyword or a literal URL. A role keyword still resolves via `GetAndCheckEnvironmentInfoAsync` (config lookup + Prod-type safety check); anything else is resolved directly via `FlowlineValidator.Default.GetEnvironmentInfoByUrlAsync`, matching how `DeployCommand.ResolveTargetUrl` falls through to the raw target string.
...
- **KTD4. Promote `DeployCommand.ParseSolutionXml` (currently `private`, `DeployCommand.cs:221-238`) into a shared static method reusable by both `DeployCommand` and the new `drift` command**, rather than duplicating its `ComponentClassifier.ParseSolutionXmlComponents` + `ScanEntitySubcomponents` + `FlowlineException`-translation logic in the new command. **Revised post-review (see KTD12): neither `DeployCommand` nor `DriftCommand` calls `ParseLocalSource` directly anymore** — `OrphanCleanupService.CompareAsync` calls it internally now, since parsing committed source is something the comparison engine needs, not something either caller should be responsible for. `ComponentClassifier.ParseLocalSource` is still the shared method this KTD promoted (renamed to take `packageSrcRoot` directly, matching every other local-source scan on that class — see KTD12).
...
- **KTD8. Reuse `ExitCode.ValidationFailed` (`15`) for "drift found."** Its existing doc comment already reads "Validation failed: drift detected, missing dependencies, or schema mismatch" (`src/Flowline.Core/ExitCode.cs:41-42`) — this case was anticipated. `ExitCode.Success` (0) covers "compared and found nothing"; existing connection/config codes (`ConnectionFailed`, `ConfigInvalid`, `NotAuthenticated`) propagate unchanged from the shared resolution/connection helpers. One code covers both `drift prod`/`drift test` (drift-detection) and `drift dev` (pre-sync preview) findings — accepted as intentionally coarse for this round rather than adding a second code to distinguish them. **Revised during code review:** a new `ExitCode.Inconclusive` (`19`) was added after all — `CompareAsync`'s early-return guards (empty local/live component sets) were found to return the same empty result as a genuinely-verified zero-drift comparison, which would have let `drift` silently report "no drift" when the comparison was actually skipped. `CompareAsync` now returns a `CompareResult(Entries, Skipped)` record so `DriftCommand` can map a skipped comparison to `Inconclusive` instead of `Success`.
...
- **KTD12 (post-review, in two rounds). `OrphanCleanupService` owns parsing committed source and is the sole authority on `PostDeployContext`'s shape — neither `DeployCommand` nor `DriftCommand` parses anything or holds parsed components themselves.**
  - **Round 1:** `PostDeployContext` dropped `LocalComponents`/`EntityLogicalNames`/`NamedComponents` (confirmed unused by the other two `IPostDeployService` implementers, `SolutionCheckService` and `BackupService` — only `OrphanCleanupService` ever read them). `ComponentClassifier.ParseLocalSource` was renamed to take `packageSrcRoot` directly (matching every other local-source scan on that class — `ScanCustomApiNames`, `ScanBotSchemaNames`, etc. — instead of being the one outlier wanting the parent folder), and `CompareAsync` calls it internally from `context.PackageSrcRoot`. The 59 existing `OrphanCleanupServiceTests` call sites needed zero changes to their assertions or arguments — only the `Ctx()` test helper changed, writing a synthetic Solution.xml fixture instead of injecting pre-parsed components directly, so the internal parse round-trips back to what each test intended.
  - **Round 2:** the three-overload shape was further revised so the comparison engine's own signature doesn't privilege `PostDeployContext` (a deploy-pipeline type that still carries `PackagePath`, which comparison never reads) as "the real implementation." The primitives-based overload — `CompareAsync(string packageSrcRoot, IOrganizationServiceAsync2 service, string solutionName, string environmentUrl, RunMode mode, CancellationToken ct, string? noDeleteHint = "(--no-delete active)")` — is now where the ~180 lines of actual comparison logic live. Both public entry points are thin: `CompareAsync(PostDeployContext context, ct, hint)` unpacks the context's five needed fields and delegates; `CompareAsync(string packageFolder, service, solutionName, environmentUrl, ct, hint)` (`DriftCommand`'s convenience overload, unchanged in purpose) computes `packageSrcRoot` and delegates with `RunMode.NoDelete` hardcoded. The two public overloads differ in arity (7 params vs 6), so there's no overload-resolution ambiguity despite both having a bare `string` first parameter with different meaning (`packageSrcRoot` vs `packageFolder`) — a deliberate, accepted tradeoff (matching every other path in the codebase, which is a plain `string`, not a typed wrapper) rather than introducing a strongly-typed path wrapper scoped to only this file.
  - `BuildManualEntriesAsync` and `BuildLocalIdentifierHarvest` (private helpers `CompareAsync` calls) were also changed to take `packageSrcRoot` directly instead of a `PostDeployContext`, for the same reason — full decoupling from the deploy-pipeline type, not a partial one.
...
- Exit codes: `Success` (compared, no drift), `ValidationFailed` (drift found), `Inconclusive` (comparison was skipped — empty local or live component set), existing codes for connection/config failures.
