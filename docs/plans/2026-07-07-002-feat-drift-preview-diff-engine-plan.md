...
- **KTD4. Promote `DeployCommand.ParseSolutionXml` (currently `private`, `DeployCommand.cs:221-238`) into a shared static method reusable by both `DeployCommand` and the new `drift` command**, rather than duplicating its `ComponentClassifier.ParseSolutionXmlComponents` + `ScanEntitySubcomponents` + `FlowlineException`-translation logic in the new command. **Revised post-review (see KTD12): `DriftCommand` itself no longer calls `ParseLocalSource` directly** — it calls `ParseLocalSource` internally via the new `OrphanCleanupService.CompareAsync(packageFolder, ...)` overload. `ComponentClassifier.ParseLocalSource` is still the shared method this KTD promoted; `DeployCommand` still calls it directly (it also needs the parsed result for nothing else, but its `PostDeployContext` construction happens inline with packing, unlike `DriftCommand`'s simpler flow).
...
- **KTD12 (post-review). `OrphanCleanupService` gets a convenience overload — `CompareAsync(string packageFolder, IOrganizationServiceAsync2 service, string solutionName, string environmentUrl, CancellationToken ct, string? noDeleteHint = null)` — that parses committed source and builds `PostDeployContext` internally, instead of `DriftCommand` calling `ComponentClassifier.ParseLocalSource` itself and threading the three parsed values through.** Caught in review: the original shape made every caller of the comparison responsible for parsing, even though only `OrphanCleanupService` ever reads the parsed fields (`PostDeployContext.LocalComponents`/`EntityLogicalNames`/`NamedComponents` — confirmed unused by the other two `IPostDeployService` implementers, `SolutionCheckService` and `BackupService`). The tempting full fix — deleting those three fields from `PostDeployContext` and having `CompareAsync(PostDeployContext, ct)` parse internally from `context.PackageSrcRoot` — was rejected: `PostDeployContext` is constructed directly (with pre-parsed, in-memory local components, no filesystem I/O) by 59 existing call sites across `OrphanCleanupServiceTests.cs`, the incident-scarred suite KTD5/KTD6 require staying at zero modified assertions. Changing `PostDeployContext`'s shape would force rewriting all of them into filesystem-fixture-based tests for no behavioral gain. The overload gets the same outcome for the one caller that actually wants it (`DriftCommand`, which has no packing step or `RunMode` choice of its own) without touching `PostDeployContext` or any existing test. `DeployCommand` is unchanged — it still builds `PostDeployContext` itself, since packing happens between parsing and context construction there, and reordering that is out of scope.
