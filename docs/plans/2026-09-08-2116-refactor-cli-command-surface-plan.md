...
- R8. Role inference, first hit wins: URL host suffix `-dev`, `-test`, `-uat` before the `.crm` segment; then environment type, Production to `prod`, Developer to `dev`. A Sandbox with no suffix hit is not inferred, because Dataverse reports a test or UAT sandbox with the same type as a DEV one: a non-interactive run fails with exit 15 naming the `.flowline` key to add for the role the URL belongs to, and an interactive run offers the role picker with no pre-selected role.
...
- AE2. **Covers R7, R8, R11.** Given `.flowline` has an empty DevUrl and no TTY, when `flowline push --env https://contoso-dev.crm4.dynamics.com` runs, then the push proceeds and prints "Saved to .flowline: DevUrl (inferred from URL suffix)", and a second run with no `--env` uses it without printing that line.
