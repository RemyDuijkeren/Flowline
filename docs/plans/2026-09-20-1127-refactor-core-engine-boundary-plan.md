...
**Outside this work.** `Flowline/Logging/` and `Flowline/Diagnostics/` stay in `Flowline` in this plan. The Serilog enrichers and the OpenTelemetry processors configure the composition root, and the exporter wiring carries packages Core does not have.

That rationale no longer covers every file in those folders, because the telemetry work landed after this plan was written. `Logging/FlowlineScrubber.cs` (189 lines) and `Logging/TelemetrySaltStore.cs`, and `Diagnostics/PacTelemetrySetting.cs`, `TelemetryConsent.cs` and `TelemetryDisclosure.cs`, carry no CLI, Serilog or OpenTelemetry dependency and would pass R1 and R6 today. They are deliberately deferred rather than absorbed: they are days old, this plan's scope was agreed before they existed, and a boundary refactor is not the place to re-open freshly shipped code. Worth a follow-on pass once this one lands.
...
| `Diagnostics/` | already exists and already holds `FlowlineActivitySource`; nothing to move here |
