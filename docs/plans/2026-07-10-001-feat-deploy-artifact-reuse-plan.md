...
- **KTD7 — DTAP gate requires an exact version match, not just "predecessor is at least as new."** Found in code review: the pre-existing gate only blocked promoting a version the predecessor tier hadn't seen yet (`predVer < localVer`), never the reverse — deploying an older version than what the predecessor already verified passed silently. Low-risk when the version always came from git (`ValidateGitCleanAsync` meant reaching it required deliberately checking out an old commit), but `--path` decouples the version from git entirely, turning a deliberate-only edge case into an easy accident (e.g. a stale build artifact). `DtapVersionMatches` (`DeployCommand.cs`) now requires exact equality. Deliberate hotfix-style downgrades are a separate future feature, not supported here; `--skip-dtap-check` remains the manual override until that exists.
