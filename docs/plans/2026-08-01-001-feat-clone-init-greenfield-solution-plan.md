...
- R9. When no environment is specified, prompt with a tenant-wide environment picker that frames the choice as selecting this project's **DEV (source-of-truth)** environment, listing the tenant's environments with their type. For the chosen environment, switch to an existing matching pac auth profile; Flowline never creates an auth profile or launches an interactive login — if no matching profile exists, it errors naming the `pac auth create` command to run.
...
- R11. When cloning an existing solution with none named, list the *unmanaged* solutions in the chosen environment for the user to pick (via `pac solution list`), excluding managed ones with a note of how many were hidden — clone supports unmanaged only.
- R17. When an interactively-picked environment is used to clone an *existing* solution (not create), Flowline confirms which `.flowline` role it saves under, defaulting to DEV when no role is configured — so later role-based commands (`push`/`sync`/`deploy`) resolve it correctly.
...
- AE3. **Covers R8, R10.** **Given** a non-dev environment — chosen interactively **or** passed via `--dev` — **when** create is attempted, **then** it is refused after checking the environment type; a dev environment proceeds and becomes the DEV role on success.
...
- **Approach:** When no solution is named and the session is interactive: resolve env (U4); list unmanaged solutions (`GetSolutionsAsync`, filter `IsManaged == false`, note the hidden-managed count, R11) in a `SelectionPrompt` plus a "create new" choice that calls the shared `SolutionCreateFlow` (U5) — the same orchestrator `init` uses, so name validation (U2) and record create run identically for clone-initiated creates. For an existing-solution clone via the picker, confirm the `.flowline` role, defaulting to DEV (R17). No TTY with no solution keeps today's error path.
