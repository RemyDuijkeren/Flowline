...
- **A managed deploy may unpack differently.** The unpack call passes `sln.IncludeManaged`, and the check assumes `pluginpackages/<uniquename>/package/<name>.nupkg` in both shapes. Verified for unmanaged on 2026-08-19; not verified for managed. If managed differs enough that no `pluginpackages/` directory exists at all, `RunPostImportAsync` takes its early return (`PostDeployOutcome.Clean` — silent, not a finding, not R7's inconclusive) rather than a false finding, but the manual round should cover a managed target.
