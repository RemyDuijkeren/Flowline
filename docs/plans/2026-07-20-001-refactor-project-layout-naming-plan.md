...
- **KD3 — `Package/` becomes `Solution/`, flat: the cdsproj and `src/` sit directly inside.** Not nested as `Solution/<SolutionName>/`. Flat is what one `Directory.Move` of `pac`'s output produces, keeps paths shorter (unpacked solutions carry long `Other/Customizations/…` paths and Windows `MAX_PATH` is a known irritant), and — because `Solution/` stays a fixed name — keeps `PackageFolder()` (`FlowlineCommand.cs:29`) a trivial static whose ~15 call sites are a rename rather than a signature change. Contoso's nested `solution/<SolutionName>/` earns its level with three solutions; at N=1 it buys nothing. Multi-solution repos get their nesting from domain folders instead (`docs/others/folder-structure-analysis.md` §5).
...
- **KTD3 — `PackageFolder()` stays a constant-derived static.** Because `Solution/` is a fixed role-based name (KD3), the helper at `src/Flowline/Commands/FlowlineCommand.cs:29` keeps its signature and its ~15 call sites are a rename, not a threading change. Only the cdsproj *filename* becomes variable, at three sites — `CloneCommand.cs:65`, `SyncCommand.cs:61`, `DeployCommand.cs:368` — and that is what solution-file discovery resolves. A nested `Solution/<SolutionName>/` would have made the folder itself solution-dependent and pushed the change into every call site; this is a concrete reason the flat shape was chosen.
