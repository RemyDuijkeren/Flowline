...
- **KD2 — Confirmation signal reuses the existing `IPlugin`/`CodeActivity` reflection filter.** A candidate project is a "plugin project" only if its build output contains a type deriving `Microsoft.Xrm.Sdk.IPlugin` or `System.Activities.CodeActivity` — the exact filter `PluginAssemblyReader.cs` already applies per-DLL for the NuGet path (`IsDerivedFrom(type, "Microsoft.Xrm.Sdk.IPlugin")` / `"System.Activities.CodeActivity"`, `PluginAssemblyReader.cs:241-242`), just applied per-project across the solution instead of assumed for one fixed folder. A project that reflects to zero matching types (WebResources, Entities, shared DTOs) is silently not a plugin project — no separate exclusion list needed.
...
- `src/Flowline.Core/Services/PluginAssemblyReader.cs:241-242` — existing `IPlugin`/`CodeActivity` reflection filter, reused as-is (KD2).
