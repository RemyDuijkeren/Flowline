...
- Command-level coverage for the paths a pure helper cannot reach. The 2026-09-10 code review found five: the non-interactive picker fallback listing names and failing (R9); the override warning firing through the command rather than only through its predicate (R4, R15); the bare branch form rendering its groups and throwing (AE9); the dry-run wording on a component write (AE2, KTD11); and a component operation outside a project failing on the missing solution flag. The last was a real bug, fixed and verified by hand from a Release build, but it has no regression test. These need a console fake driving a command rather than a helper, which is a test-harness shape this project does not have yet.
