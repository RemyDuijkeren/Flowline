...
- R6a. **Superseded by live testing — kept for history, not current behavior.** Originally: an explicit dotted `Function` (e.g. `MyCompany.Example1.OnLoad`) was treated as already fully qualified, skipping namespace derivation and its prefix entirely. Live testing (post-implementation) found two gaps this created: (1) a wrong or typo'd prefix silently matched an unrelated same-named export in a different file's own build output (e.g. `Example1.onload1` written inside `example2.js` matched `example2.js`'s own `onLoad1` export, because only the tail was ever checked), and (2) the filename-guessed namespace (`ToPascalCase(filename)`) doesn't account for Rollup's own `namespacePrefix` setting, so a project using it (e.g. `this.Test.Example2 = ...`) had every *defaulted* handler registered under the wrong, nonexistent global path too — not just an explicit-name edge case. Since a Handler's `libraryName` is always the file the annotation lives in, a referenced function can only ever live in that same file — so **the dotted prefix (and the defaulted namespace) must now match this file's own real namespace, read structurally off the built file's own IIFE invocation** (not guessed from the filename, and never by reading `rollup.config.mjs`); a mismatch is a hard fail (folded into R7a's outcome 2), not a fully-qualified escape hatch. This is now purely about whether `Function` was written explicitly by the user (with or without a namespace prefix) vs. defaulted (R4) — R7a governs what happens either way. See `FormEventFunctionResolver.Resolve`'s class doc comment (and its `FindActualNamespace` helper) for the current mechanism.

**Validation**

- R7. **When `Function` is omitted** (defaulted to `onLoad`/`onSave` per R4) and the default name does not resolve in the file, `push` fails with a clear error naming the file and the missing function; no handler is registered for that declaration, but other valid registrations in the same push still apply. This is Flowline's own inferred guess failing, not a user assertion — hard fail is correct here, **on any non-resolution, not only a positively-proven absence** (unlike R7a — see below). R7a's "absence must be a positive determination" rule governs the split *within* R7a's explicit-name case only; it does not extend to R7, because R7's defaulted guess has no explicit user assertion to fall back on when the parser can't fully trace the export surface — the escape hatch for that situation is R7a itself (name the function explicitly instead of relying on the default).
- R7a. **When `Function` is explicitly written by the user** (including a dotted/qualified form, e.g. `Example1.OnLoad`, whose prefix must match this file's own auto-derived namespace — see R6a), Flowline verifies it against the real parsed structure (U8, Acornima AST-walk) and distinguishes three outcomes, not two:
  1. **Found** — the name resolves in the parsed export surface → register silently.
  2. **Confirmed absent** — either Acornima fully traced a *known* export shape (the Rollup `exports.X =` IIFE assignment, or a verbatim bare-global declaration) and the name provably is not there, **or a dotted name's namespace prefix does not match this file's own auto-derived namespace** (R6a) → `push` **hard fails**, same failure style and same per-declaration scope as R7 (fails with a clear error naming the file and the missing function; no handler is registered for that declaration, but other valid registrations in the same push still apply — a bad explicit name never aborts the whole push). Once the parser has positively proven the function doesn't exist (or the prefix doesn't belong to this file), an explicit user assertion doesn't change that — this is no longer "an assertion Flowline merely couldn't confirm," it's a proven mistake (a typo that would otherwise fail silently at runtime, since the handler would simply never fire in the browser).
  3. **Inconclusive** — the dotted name's prefix matches this file's own auto-derived namespace, but the parser could not fully trace the file's export surface for the tail (e.g. namespace objects assembled across multiple statements, a build shape outside the two known patterns) → `push` **warns** (not fails) that the name could not be confirmed and registers it anyway. Flowline's own detection is a fallback, not a source of truth, so when it genuinely can't determine the answer for a name it has already confirmed belongs to this file, the user's explicit assertion still wins.
...
**Superseded mechanism, empirical finding still valid (see KTD13):** the original plan had `FormXmlEventSerializer`'s function-existence check try both patterns above via regex. KTD13 replaces the regex mechanism with an embedded JS engine for two reasons the regex approach could not address: (1) a comment or string literal containing lookalike text (`// old: exports.onLoad = ...`) can false-positive-match a regex but not a real parse, and (2) the regex has no way to walk a genuinely nested/prefixed namespace or validate a dotted name's prefix against it (R6a/R7a). The `exports.<Name> = ` shape confirmed here is still the correct signal to look for — KTD13 just looks for it structurally instead of textually.
...
**KTD13 — Function resolution uses a real JS parser (Acornima, structural AST-walk, no execution), not regex, and validates an explicit dotted name's namespace prefix against the file it's declared in.** Two problems regex can't solve cleanly: (1) a comment or string literal containing lookalike text (`// exports.onLoad = oldThing`) can false-positive-match a text pattern but not a structural parse; (2) regex has no way to verify a genuinely nested/prefixed namespace path or confirm a dotted name's prefix actually belongs to this file (R6a).
...
**Explicit-name resolution (R6a/R7a):** when the annotation's `Function` is non-null (user wrote something, whether dotted or not — R4's default is the only case treated as a guess), AST-based verification is no longer purely advisory. Because Acornima performs a real structural parse (not regex), it can sometimes *positively prove* a name absent from a fully-traced known export shape, not just fail to find it. A dotted name's prefix must match this file's own auto-derived namespace (R6a) — a mismatch is treated the same as a provably-absent tail (R7a outcome 2), since a Handler's `libraryName` is always the file the annotation lives in, so a referenced function can only ever live in that same file. R7a's three-outcome resolution (found / confirmed absent / inconclusive) treats confirmed-absence the same as R7's default-case hard gate, and reserves the warn-and-register-anyway behavior for genuine inconclusiveness (a matching-prefix name whose tail the AST-walk can't fully trace). See R7a for the full rationale and U8's `Resolve` method for the mechanism.
...
*Why enrich the messaging (R18a/R18b):* a handler on a tracked library with a non-matching ID is ambiguous by construction — it could be genuinely stale (safe to remove) or a legacy/migration-era registration nobody has annotated yet (removing it is actively harmful). There is no reliable way to auto-detect which situation a given handler is in: the distinguishing fact ("did this handler ever have a Flowline annotation that was later removed, or did it never have one") is exactly the kind of history a local tracking file would capture, and that was already rejected during the original brainstorm (no separate state, everything derivable from source). Rather than guess, surface enough information for the human to decide well: the handler's own stored `functionName`/`libraryName`/`parameters` already fully determine a proposed annotation (R18a), reusing the stored `functionName` verbatim (including its dotted form, if any). This is *not* guaranteed to resolve if adopted — a legacy or migration-era handler's stored dotted name may have a prefix that doesn't match this file's own auto-derived namespace (R6a), in which case adopting the proposed annotation verbatim would hard-fail on the next `push` (R7a outcome 2) just as surely as removing it would have. `--dry-run` gets the same detail (R18b) so this is discoverable with zero risk before a real `push`.
...
- `FormEventHandler(string FunctionName, string LibraryName, Guid HandlerUniqueId, string Parameters)` record — mirrors `DependencyLibrary`'s Name-only-equality-override pattern (KTD8), but keyed on `(FunctionName, LibraryName)` per R12's existing dedup key.
- `FormLibrary(string Name, Guid LibraryUniqueId)` record — same equality pattern, keyed on `Name`.
...
- `FormEventHandler`/`FormLibrary` `HashSet` membership: two instances with same key but different other fields (e.g. different `Parameters`) are treated as equal / same hash bucket.
...
- `IReadOnlySet<FormEventHandler> GetHandlers(XDocument form, FormEventType evt)` — reads `<events>/<event name="onload"|"onsave">/<Handlers>/<Handler>` (never `<InternalHandlers>`), returns the current set.
- `IReadOnlySet<FormLibrary> GetLibraries(XDocument form)` — reads `<formLibraries>/<Library>`.
- `void SetHandlers(XDocument form, FormEventType evt, IReadOnlySet<FormEventHandler> desired)` — finds or creates `<events>` (root-level child of `<form>`), finds or creates `<event name="..." application="true" active="true">` when absent (see Risks for the `application`/`active` default choice), finds or creates `<Handlers>` inside it, and replaces its `<Handler>` children with one element per desired entry: `<Handler functionName="..." libraryName="..." handlerUniqueId="{guid}" enabled="true" parameters="..." passExecutionContext="true"/>` (R13 fixes `enabled`/`passExecutionContext` to `true`). Leaves `<InternalHandlers>` and every other event untouched.
- `void SetLibraries(XDocument form, IReadOnlySet<FormLibrary> desired)` — finds or creates `<formLibraries>`, replaces `<Library>` children analogously.
...
**Goal:** Replace regex-based function-existence checking with real structural JS parsing (Acornima AST-walk, no execution), and implement explicit-name resolution with namespace-prefix validation (R6a/R7a).
...
2. If `isExplicit` (R7a — the user wrote a name, whether dotted (R6a, always explicit) or bare): if dotted, the prefix must match `autoNamespace` (case-insensitive) — a mismatch resolves immediately to `Found: false` with whatever `Confident` the file's overall export-shape trace already established (R6a; a wrong/typo'd prefix, or a reference into an unrelated file, must never silently match a same-named export elsewhere in this file). Once the prefix matches (or the name is bare), attempt namespaced-then-bare matching against the tail, same as branch 1, but under R7a's rules, not R7's. `Confident` reports whether Acornima fully traced a *known* export shape (the `exports.X =` IIFE assignment, or a verbatim bare-global declaration) for this file — i.e. whether absence, if reported, is a positive determination rather than a shape the walk couldn't fully enumerate. Combined with `Found`, this yields R7a's three outcomes: `Found: true` → register silently; `Found: false, Confident: true` → **confirmed absent, R7a hard-fails** (same handling as R7's `Found: false`); `Found: false, Confident: false` → **inconclusive, R7a warns and registers anyway**.
...
- Explicit dotted request (`Example1.OnLoad`, prefix matching `autoNamespace: "Example1"`) where the built content's structure confirms the tail exists → `Found: true, Confident: true` — registers, no warning (R7a happy path, outcome 1).
- **Explicit request (dotted with a matching prefix, or bare) for a name that provably does not exist in a fully-traced known export shape** (e.g. `exports.onLoad = onLoad;` present and enumerable, but the requested tail matches nothing in it) → `Found: false, Confident: true` — R7a's new hard-fail outcome (outcome 2): the caller must fail the push, not warn-and-register.
- **Explicit dotted request whose prefix does not match `autoNamespace`** (e.g. `Example1.onLoad1` requested against a file whose auto-derived namespace is `Example2`, even when that file happens to export a same-named `onLoad1`) → `Found: false` — R7a's hard-fail outcome (outcome 2), never a silent cross-file match on the tail alone (R6a).
- Explicit dotted request with a matching prefix where verification is inconclusive for the tail (e.g. namespace object assembled across multiple statements, or a build shape the AST-walk doesn't recognize, so the parser cannot fully trace the export surface) → `Found: false, Confident: false` — R7a's warn outcome (outcome 3): registers anyway, caller must surface a warning, does not hard-fail.
...
3. Build the desired `FormEventHandler`/`FormLibrary` sets from resolved annotations, same as before (R17 deterministic IDs).
...
- Two annotations for the same `(entity, form, event, functionName, libraryName)` — same file, duplicate line → deduplicated, one `FormEventHandler`.
...
1. **If `cleanupOnly` (Phase 1/cleanup call — KTD12/U7):** before computing what to write for each form, replace `plan`'s per-form `DesiredHandlers`/`DesiredLibraries` with their **intersection against the form's current `Handlers`/`Library` sets, taking each survivor's value from the current set, not from desired** (read via `FormXmlEventSerializer.GetHandlers`/`GetLibraries`, using `FormEventHandler`/`FormLibrary`'s existing identity-only equality). This is the concrete mechanism for "cleanup applies removals only, never additions or in-place updates": a desired handler/library not already present on the form necessarily references a not-yet-created web resource (nothing else could have gotten it onto the form), so excluding it defers it to Phase 3 without needing any separate "does this library exist in Dataverse yet" lookup. Anything currently present and still desired survives **untouched at its current value** — even when `desired` has a different `Parameters` value for that same identity, the parameter update itself is deferred to the registration phase, not applied here; only genuine removals (current-but-not-desired: stale, or confirmed-unrecognized-for-removal) happen during cleanup. `UnrecognizedHandlers` and the confirmation gate (steps 2-3 below) are unaffected by this filter — a form can still have unrecognized handlers surfaced and confirmed in the cleanup pass; the filter only ever *removes candidates from the write*, never adds or updates one.
...
- **`cleanupOnly: true`, a desired handler is already current but with a changed `Parameters` value → nothing is written for it** (deferred to the registration-phase call, same as a brand-new handler/library) — cleanup is removals-only by design: a clean separation of concerns where cleanup only ever shrinks the current set, and registration is the only phase that adds or modifies (parameter-only changes included).
...
- Manual smoke, live dev environment (mirrors the empirical investigation already performed in this document):
  ...
  9a. Reference an explicit, dotted function name whose prefix matches this file's own auto-derived namespace (e.g. `Example1.OnLoad` in `example1.js`) that Flowline's verification genuinely can't trace for the tail (inconclusive export shape) — confirm `push` **warns but still registers** it (R7a outcome 3), does not hard-fail.
  9b. Reference an explicit function name (dotted with a matching prefix, or bare) that Acornima confirms, from a fully-traced known export shape, does **not** exist, alongside a second, unrelated, valid annotation — confirm `push` **hard-fails** naming the file and function (R7a outcome 2), does not register a handler for the bad declaration, but the second annotation still registers (per-declaration failure, same as R7 — not a whole-push abort).
  9c. Reference an explicit, dotted function name whose prefix does **not** match this file's own auto-derived namespace (e.g. `Example1.onLoad1` written inside `example2.js`, even when `example2.js` happens to export a same-named `onLoad1`) — confirm `push` **hard-fails** naming the file and function (R7a outcome 2), never silently registering a handler pointed at an unrelated file's export (R6a).
...
- `docs/WebResources-Project.md` (wiki) updated with the `// flowline:onload`/`// flowline:onsave` annotation syntax, including explicit dotted-name resolution and its namespace-prefix requirement (R6a/R7a), per Documentation Notes below.
...
- `WebResources-Project.md` (wiki) — document the `// flowline:onload`/`// flowline:onsave` annotation syntax alongside the existing `// flowline:depends` documentation, including explicit dotted-name resolution and its requirement that the prefix match the file's own namespace (R6a/R7a) — a mismatched prefix hard-fails rather than being treated as an escape hatch — and that unrecognized-handler removal is authorized via the existing `-f|--force` flag (R18c, unchanged from KTD6).
