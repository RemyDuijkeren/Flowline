...
- R6a. **Auto-namespace derivation is a fallback, not a source of truth.** It exists because the built bundle's actual namespace structure isn't reliably discoverable any other way (R6a's own detection can only guess a per-file default). A project's build tooling is not fixed — `rollup.config.mjs` is scaffold, not contract, and a project may replace it with a different bundler or a hand-written `dist/` output entirely, changing the real namespace shape out from under any config-parsing approach. When `Function` in the annotation is explicitly a dotted/qualified name (e.g. `MyCompany.Example1.OnLoad`), Flowline treats it as **already fully qualified** and skips auto-namespace derivation entirely for that declaration — see R7a for the accompanying validation behavior.
...
**Approach:** `(string FunctionName, bool Found, bool Confident) Resolve(string builtJsContent, string requestedFunctionName, string autoNamespace)`:

1. If `requestedFunctionName` is null (R4 default) or contains no `.`: unchanged auto-detection semantics from the original plan — try `autoNamespace.requestedFunctionName` then bare `requestedFunctionName`, case-insensitive, real casing from source. `Found: false` here is R7's hard-fail case.
2. If `requestedFunctionName` contains a `.` (R6a, always explicit — R4 only defaults to a bare name), OR the annotation otherwise names an explicit (non-defaulted) function with no dot: skip auto-namespace derivation, treat as already fully qualified (dotted case) or as a bare explicit name (undotted case). Attempt verification against the parsed structure. `Confident` reports whether Acornima fully traced a *known* export shape (the `exports.X =` IIFE assignment, or a verbatim bare-global declaration) for this file — i.e. whether absence, if reported, is a positive determination rather than a shape the walk couldn't fully enumerate. Combined with `Found`, this yields R7a's three outcomes: `Found: true` → register silently; `Found: false, Confident: true` → **confirmed absent, R7a hard-fails** (same handling as R7's `Found: false`); `Found: false, Confident: false` → **inconclusive, R7a warns and registers anyway**.
...
2. For each form/event, group any *current* annotations targeting it by `(Entity, Form, Event)` as before; resolve each via U8's `Resolve(...)` (replaces the old direct `FormXmlEventSerializer.ResolveFunction` call). Three outcomes drive three distinct handling paths, not two:
...
- Manual smoke, live dev environment (mirrors the empirical investigation already performed in this document):
  ...
  5. Hand-add (via maker portal) a second handler on a tracked library pointing at a completely different, unrelated web resource this project doesn't manage — confirm `flowline push` **never surfaces it, never prompts about it, and never removes it**, even with `--force` passed. Direct end-to-end proof for the original R15 gap.
