...
- **SUPERSEDED — Production/Sandbox Type is sourced from the target Dataverse environment's own `RetrieveCurrentOrganization` Web API function, not the Power Platform BAP admin API.** This plan originally chose the BAP admin API (`api.bap.microsoft.com`) for Type, reasoning the Dataverse `organization` table has no genuine Production/Sandbox field. That table check was correct, but the conclusion was wrong: `RetrieveCurrentOrganization` — a Microsoft-documented, versioned Web API function distinct from the `organization` table — returns an `OrganizationType` field (`Secondary`/`Customer` → Production, `CustomerTest`/`CustomerFreeTest` → Sandbox, etc.) that the original live verification missed. This was discovered after shipping the BAP-based implementation, when a real user hit `AADSTS65002`: the BAP admin API requires a separate, tenant-specific first-party preauthorization Microsoft grants inconsistently, making it fragile "for all kinds of users" in a way the Dataverse organization's own endpoint — already required for every other Flowline operation — is not. Implemented as `DataverseConnector.GetEnvironmentInfoAsync` calling `{environmentUrl}/api/data/v9.2/RetrieveCurrentOrganization(...)` with a token scoped to the environment itself, verified live against both a real Sandbox (Dev) and a real Production environment.
- **No new permission requirement (superseded rationale, same conclusion).** The original BAP-based rationale ("same access `pac admin list` already required") no longer applies. Under the `RetrieveCurrentOrganization` approach, no new permission is needed for a different reason: it's the same Dataverse environment access every other Flowline operation (connect, push, clone) already requires — no separate admin-scoped grant at all, which is also why it doesn't hit the BAP preauthorization gap.
...
- SUPERSEDED: BAP admin API `GET https://api.bap.microsoft.com/providers/Microsoft.BusinessAppPlatform/scopes/admin/environments?api-version=2020-10-01` returns `properties.environmentSku` (Production/Sandbox/Developer/Teams/Default) — verified live via a direct MSAL-cache token acquisition mirroring `DataverseConnector.ConnectViaPacAsync` (`src/Flowline.Core/Services/DataverseConnector.cs:25`); no `pac.exe` involved, no active-profile dependency, succeeded silently with no new interactive consent. **This source was shipped, then replaced** — see below.
- Microsoft's "Troubleshoot missing environments" doc confirms `pac admin list`'s inclusion rule: a Dataverse System Administrator role in a specific environment qualifies for that environment to appear in the caller's results, independent of tenant-wide admin rights. (No longer load-bearing once the BAP dependency was removed.)
- **Post-ship correction (superseding source):** a real `flowline clone` failed live with `AADSTS65002` against the BAP admin resource (`api.bap.microsoft.com`) for a genuine, correctly-authenticated PAC profile — Azure AD requiring a first-party-to-first-party preauthorization Microsoft hadn't granted for this tenant's use of the PAC CLI app ID against the BAP admin API specifically. Re-verified live per user direction (real REST calls against a real Dev and a real Production environment, not docs research) that `RetrieveCurrentOrganization` — a Dataverse Web API function, not the `organization` table — returns `OrganizationType` (`Secondary` for the real Production org, `CustomerTest` for the real Dev/Sandbox org), with zero BAP dependency and zero new permission requirement beyond what every other Flowline operation already needs. `GetEnvironmentInfoAsync` was rewritten accordingly.
...
**Approach (superseded — see the Key Decisions note above; implemented against `RetrieveCurrentOrganization`, not the BAP admin API):**
- Acquire a token scoped to the target `environmentUrl` itself (not an admin API) using the same cached-account lookup `ConnectUserAsync` uses for user profiles (`AcquireTokenSilent`) and the same cache-only assertion pattern `ConnectServicePrincipalAsync` uses for service-principal profiles (`AcquireTokenForClient`). Reuses `AcquireUserTokenAsync`/`AcquireServicePrincipalTokenAsync`'s existing `FlowlineException`-with-actionable-message shape on an `MsalUiRequiredException`/`MsalException`.
- Call `GET {environmentUrl}/api/data/v9.2/RetrieveCurrentOrganization(AccessType=Microsoft.Dynamics.CRM.EndpointAccessType'Default')` with the acquired token — a single-organization Dataverse Web API function, not a multi-environment admin listing, so no search/match step is needed; a non-2xx or connection failure maps to `null` directly.
- Map the `Detail` object to `EnvironmentInfo`: `Type` from `OrganizationType` (a string enum name — `Secondary`/`Customer` → `"Production"`, `CustomerTest`/`CustomerFreeTest` → `"Sandbox"`, else passed through as-is) via `MapOrganizationType`. Exact JSON property paths (`OrganizationId`, `EnvironmentId`, `FriendlyName`, `UrlName`, `OrganizationVersion`) confirmed live against a real Dev and a real Production environment.
- Extract the mapping into a pure `internal static` method (`MapRetrieveCurrentOrganizationResponse`), mirroring `PacUtils.BuildCheckResult`/`TryCountSeverities`'s existing testable-extraction pattern, so it's unit-testable without a token or network call.
- Return `null` (not throw) when the HTTP call fails (non-2xx status or `HttpRequestException`, e.g. DNS/connection failure for a nonexistent URL, or no access) — matches the existing `GetEnvironmentInfoByUrlAsync` contract callers already handle (`env == null` → `ExitCode.ConnectionFailed`).
...
- **External API surface — resolved by the supersession.** The originally-shipped BAP admin-environments endpoint was an undocumented-for-this-use, Microsoft-internal API surface requiring a separate first-party preauthorization that Microsoft granted inconsistently per tenant — this materialized as a real `AADSTS65002` production failure, not just a theoretical risk. `RetrieveCurrentOrganization` is a documented, versioned Dataverse Web API function requiring no permission beyond what every other Flowline operation already needs, removing this risk category rather than just mitigating it. The pure-mapper extraction in U1 (`MapRetrieveCurrentOrganizationResponse`) still isolates blast radius from any future Microsoft-side response-shape change, and the existing `pac.exe`-based overload stays alive for `ProvisionCommand` (KTD7) regardless.
