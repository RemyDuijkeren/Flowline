...
**Refresh note (2026-07-24): "no new scanning code needed" for Role, above, no longer holds.** A code review flagged that Role's raw id-match (unlike WebResource/Entity/OptionSet) never got a live-name fallback, so a role reconciled to a different live id in the target org on import (Dataverse's documented by-name reconciliation for security roles when a same-named role already exists there) would misclassify as orphaned. `ComponentClassifier.ScanRoleNames` now reads the local `Roles/<name>.xml` file names — the one identity shape this doc's part 8 taxonomy didn't need a scanner for turned out to need one after all, for a different reason (cross-env id drift, not local-declaration detection) — and `OrphanCleanupService.CompareAsync` folds them into the same `ResolveNamedComponentIdsAsync`/`NameResolvableTypes` (`role`/`roleid`/`name`) path WebResource already uses, unioned into `sNewIds` alongside the raw id, never replacing it (a role whose raw id still matches same-environment but whose display name changed in-target must still resolve via the id).
