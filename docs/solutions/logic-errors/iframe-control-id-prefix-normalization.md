...
2. **Default function-name derivation** — `FormEventPlanner.DeriveDefaultFunctionName`'s `OnReadyStateComplete` branch (`FormEventPlanner.cs:467-477`) builds `"onReadyStateComplete" + PascalCase(controlId)` when an annotation omits an explicit function name. Without normalizing the token first, the same control produces a different default function name depending on which spelling the annotation happened to use — `onReadyStateCompleteMyFrame` (bare) vs `onReadyStateCompleteIFRAMEMyFrame` (prefixed — `ToPascalCase` splits on the underscore and capitalizes only each word's first character, so `IFRAME_myFrame` becomes `IFRAMEMyFrame`, not a cleanly cased `IframeMyFrame`).
