...
### Handler status
A maturity level a handler declares about itself, hardcoded in its own code rather than external config. Four rungs, in ascending order of what the handler is trusted to do. **Silent** handlers run full detection and print verbose findings, but are excluded from the actionable report and never act — this lets a handler ship and be field-tested with no action risk before promotion. **Report** handlers surface real findings with a real [[Orphan priority]] verdict, but never delete; the block is policy (human review before removal), not weak detection. **Guarded** handlers delete, but only under explicit `--force delete-orphans` consent. **Auto** handlers delete with no extra gate. A component type with no handler at all falls back to the generic search-all identifier-harvest signal (verbose "possible match" only, never prioritized, never acted on).

### Provenance verdict
Where a reported [[Orphan component]] came from, derived per run from the repository's history of the [[Unpacked solution source]] and reported beside the entry. Three cases, and only these three names are canonical — "unresolved", "never-in-repository" and similar are drift. **Declared** means source once declared the component and a commit removed it; the verdict carries that commit's sha, author, date and subject, and the subject is the stated reason for removal. It says nothing about the environment — only that source no longer declares it. **NeverInSource** means no commit in the reachable history ever declared it, on a checkout complete enough for that absence to be evidence. **Undetermined** means the lookup could not answer: no [[Local-source identity shape]] mapping, a shallow or partial checkout, an unrebasable path, or an ambiguous result. Undetermined is never rendered or treated as NeverInSource — a failed lookup shown as "never yours" would mark a real deletion safe to ignore, which is the inverted-safety failure the verdict exists to prevent. The verdict is reported only: it changes no [[Handler status]], makes no component newly deletable, and moves no exit code. Nothing is cached between runs, so a failed lookup can never harden into an answer.
...
### Environment role
One of the four named places a Flowline project addresses: `dev`, `test`, `uat`, `prod`. A role is a key in `.flowline` holding one environment URL, and it is the value a command accepts wherever it names an environment, either as a positional target (`deploy prod`) or through `--env`, which names the existing environment the command connects to. `push`, `pull` and `init` accept only `dev`; `provision` accepts only `prod`, the source it copies from; the others accept any role or a URL.

### Role inference
The rule that assigns an [[Environment role]] to a URL that is not yet in `.flowline`, so the URL can be saved under a key without the user naming one. In order: a Production type is `prod` whatever the name says; then a keyword in the URL's host label (`dev`, `develop`, `development`; `test`, `tst`, `testing`, `qa`, `sit`; `uat`, `acc`, `acceptance`, `acceptatie`, `preprod`, `staging`, `stage`, `stg`), matched as a whole hyphen-separated token in any position with trailing digits ignored, last match wins; then the same keywords in the environment's display name; then a Developer type is `dev`. A Sandbox with no keyword hit is not inferred, because Dataverse reports a test or UAT sandbox with the same type as a DEV one. Interactive runs show a picker, with the inferred role pre-selected when there is one; non-interactive runs save an inferred role and print which key was written and why, and fail naming the key to add when nothing could be inferred.
...
### Custom API ownership inversion
A Custom API and a plugin step both carry a `plugintypeid`, but the reference runs in opposite directions, and conflating the two is the single recurring source of wrong deletions in `push`. A step is **owned by** its plugin type — `sdkmessageprocessingstep.plugintypeid` points *up* at its owner, deleting the type cascades the step away, and the type is assembly-scoped, so "the type is gone" genuinely means "the step is dead". A Custom API **references** a plugin type as its implementation — `customapi.plugintypeid` points *down*, the API is the parent, and it can exist with no `plugintypeid` at all. Losing an implementation therefore does not orphan a contract. Compounding this, a Custom API has no assembly-shaped handle, so Flowline can only query them publisher-wide (`uniquename BeginsWith "<prefix>_"`) — the candidate set always includes other projects' and other repos' APIs. A Custom API is also public API surface with external callers, so the bar for deleting one is higher than for a step. See [[Positive attribution]] for the rule this forces.
...
### SolutionFileLayout
The resolved view of a project's solution file (`.sln`/`.slnx`) — the solution-file analog of `ProjectConfig` (`.flowline`'s owner). Reads the solution file exactly once per load and exposes every project Flowline resolves from it: the [[Dataverse solution project]], the plugin projects, and the WebResources project (`src/Flowline.Core/Services/SolutionFileLayout.cs:31-105`). Per-type detection is delegated to focused resolvers it composes rather than owning the rules itself — an exactly-one-`.cdsproj` rule for the Dataverse solution project, a framework-plus-reflection rule for plugins, an elimination-plus-content-signals rule for WebResources — so each rule is stated once and unit-tested without a command. Resolution is lazy per project type and cached (`SolutionFileLayout.cs:24-29`), so a command pays only for the validation the projects it actually reads require. A folder with no solution file at all is the one precondition every command but `clone` shares, and is an error rather than a silent fallback (`SolutionFileLayout.cs:95-105`).

### Dataverse solution project
The `.cdsproj` project and, under `src/`, the packed and committed mirror of a solution's Dataverse content that `deploy` always packs and imports from — the single artifact of record for what a given deploy actually ships, at every pipeline stage from a first TEST promotion through PROD. Distinct from a project's own local build output (e.g. a web-resources build's output folder): the two are expected to hold equivalent content after a sync, but only the Dataverse solution project's folder is guaranteed to exist and be current on a build agent that never runs the local build step. A [[Post-deploy service]] that needs to inspect "what's actually being deployed" must read from there, never from a separate build artifact.
...
The per-environment state a solution import cannot carry — environment variable values, connection references, flow and classic workflow activation, plugin step enablement — stated in a git-tracked file per environment and reconciled into the environment by `flowline settings push`. One component can also be changed directly with `flowline settings flow|workflow|plugin|envvar|connref`, which writes the environment and not the file, so the change is deliberate drift the command warns about. It is a *partial* declaration: the components the file names are reconciled and every other component is left untouched, one rule for every component class. The platform already defaults to on — a deploy passes `--activate-plugins`, which activates plug-in steps and classic workflows, and a first-time import activates cloud flows that were on at export — so the file carries what the platform would get wrong, chiefly a component switched off after a deployment that should be on. Distinct from state restoration, which preserves whatever the target already had across an import instead of declaring what it should be; the two can coexist and neither implies the other. Values are literal, so a settings file is as sensitive as the environment it was captured from. Which values are sensitive cannot be inferred from the component type — a plain-string environment variable can carry a secret, while a Secret-type one carries only an Azure Key Vault reference — so secret indirection is planned as a separate capability rather than a rule keyed on type. [[Secure Configuration]] is the one class that can never round-trip: it is never returned on read, so it can be applied but never captured.
