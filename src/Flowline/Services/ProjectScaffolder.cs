...
public class ProjectScaffolder(IAnsiConsole console, SubprocessCapture capture)
{
    ...
    internal static string BuildAgentsFileContent(string solutionName, string slnFileName, string dataverseSolutionFolderName)
    {
        ...
        [
            ...
            ("docs/", "not scaffolded by clone; created on first `flowline pull` (DATAVERSE_CONTEXT.md)"),
        ...
        return $$"""
            ...
            flowline pull                   # pull solution state from DEV, bump version, unpack to XML
            ...
            - Always run `flowline push` before `flowline pull` when plugin code changed.
            - `flowline pull` requires no uncommitted changes in `{{dataverseSolutionFolderName}}/src/` (exit code 12 if dirty).
            - `flowline deploy` requires no uncommitted changes under the target solution's folder (exit code 12 if dirty).
            - DEV is the source of truth. Pull captures its state; never hand-edit unpacked XML.
            - `clone`, `push`, and `pull` require an unmanaged solution in DEV — they fail on managed environments.
            - Managed/unmanaged mode is set once via `clone --managed`/`pull --managed`; `deploy` always uses the solution's configured mode.
