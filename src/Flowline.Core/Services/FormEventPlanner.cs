...
public class FormEventPlanner(IAnsiConsole console)
{
    ...
    public FormEventSyncPlan Plan(FormEventSnapshot snapshot, bool suppressWarnings = false)
    {
        ...
        foreach (var dataverseForm in snapshot.Forms.Values)
        {
            ...
            // Same union pattern as onChangeAttributes, extended to Tab/IFRAME scopes (R5): live
            // container-scoped events with at least one Handler, plus scopes referenced by a current
            // annotation — either alone would miss orphan cleanup for the other side, same rationale as R13.
            // Unlike onChangeAttributes, an annotation-referenced Tab/IFRAME scope must actually resolve to
            ...
            foreach (var (evt, attribute) in planningKeys)
            {
                ...
                // Shared by the R11 and R12 error messages below.
                var scopeDescription = attribute is null ? $"{entity}.{form}.{evt}" : $"{entity}.{form}.{evt}.{attribute}";

                // R6: BehaviorInBulkEditForm is OnLoad-only — the union is recomputed from scratch on every
                // push (KTD4), not merged with the form's prior FormXml state, so a form whose last
                // [bulkEdit] annotation was removed correctly clears the attribute (R8).
                ...
                // ordered list (not a set) so each entry's [order:N]/encounter-position survives to the sort
                // step below (KTD2). Always computes a fresh deterministic HandlerUniqueId, so a
                // matched-by-identity entry with changed Parameters naturally lands here with the new
                // Parameters value (no separate "update" bucket needed).
                ...
                foreach (var resolved in relevantAnnotations)
                {
                    ...
                    {
                        // R7: [bulkEdit] is only meaningful on onload — reject rather than silently ignore,
                        // same per-annotation isolation as the function-not-found failure below.
                ...
                // R11: two annotations sharing this event/scope that both specify the same [order:N] value
                // is a conflict regardless of whether they resolve to the same handler identity — name every
                ...
                // today's existing first-encountered-wins behavior — not a new conflict class this plan
                // validates (KTD2); DistinctBy already keeps the first occurrence per key, mirroring the
                // dedup a HashSet.Add performed before this change.
                // R9/R10: explicit [order:N] entries sort ascending first; unordered entries (Order: null)
                // keep their annotation-encounter order and append after — OrderBy's documented stability
                // preserves relative order among the equal (null) keys.
                ...
                // event's Handlers set on write). Kept in FormXml document order (not annotation-ordered —
                // these have no [order:N] of their own), same as unrecognized below.
                ...
                // Parallel ordered list of the same handlers, sourced from the same currentHandlers iteration
                // as `unrecognized` above — needed because HashSet<UnrecognizedHandler>'s enumeration order
                // depends on string.GetHashCode, which .NET randomizes per process. Reading order back out of
                // `unrecognized` itself would make desiredHandlers' order (and therefore the KTD8 orderChanged
                // check below) vary across separate `flowline push` runs even with byte-identical input.
                ...
                // Interleave-preserving merge (product decision, adversarial-review finding): a foreign or
                // unrecognized handler — most importantly a Microsoft OOB script — must keep its exact
                // original position in the live FormXml, never getting silently relocated to run after a
                // Flowline-managed handler it used to run before. Walk currentHandlers in its original
                // document order: a foreign/unrecognized entry is an "anchor" and is placed exactly where it
                // already was; every other position (a Flowline-managed handler being kept/updated, or a
                // stale one being dropped) is a "Flowline slot", filled from annotationDesired in its own
                // newly-computed R9/R10 order. Only brand-new Flowline handlers with no prior slot — and any
                ...
                // R12: the full set this event/scope will actually write — Flowline-managed plus
                // foreign/unrecognized pass-through — must not exceed Dataverse's 50-handler-per-event cap.
                // Checked before any Dataverse write (the whole Plan() call fails before FormEventExecutor
                // ever runs, per the accumulate-then-throw pattern below).
                ...
                perEventResults.Add(new PerEventPlanResult(evt, attribute, desiredHandlers, unrecognized, currentHandlers, managedLibraryNames, bulkEditEnabled, bulkEditChanged));
            }
            ...
            {
                // KTD8: FormEventHandlerDiffer.Diff only detects added/updated/removed by identity and
                // parameter state — a pure [order:N] reorder with no membership/parameter change diffs to
                // (0,0,0) and would otherwise be skipped entirely, silently discarding the corrected order.
                ...
                // R8: a bulk-edit-only change touches neither DesiredHandlers nor its order, so it must
    ...
    // ordered lists to sets here (KTD8): Diff stays scoped to content-equality only, so a pure reorder never
    ...
    // `// flowline:onload/onsave <entity> "<form>" Function[(params)]` shape R1 defines, or — for any
    // attribute-scoped directive (onchange/tabstatechange/onreadystatecomplete) — the
    // `// flowline:<directive> <entity> "<form>" <scope> Function[(params)]` shape (KTD6). attribute is
    ...
    // R3: Tab/IFRAME default names never strip a publisher prefix — tab and IFRAME control names are
