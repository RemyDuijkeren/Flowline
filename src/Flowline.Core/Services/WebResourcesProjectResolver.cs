...
/// (<see cref="PcfProjectResolver.IsPcfProject"/>, R10), and isn't a test project. With R5 requiring
/// exactly one WebResources project, a single survivor of that filter *is* the WebResources project by
/// elimination — the signals below exist only to rank when more than one survives.
///
/// <b>Weighted signals only break ties among survivors.</b> Each candidate's own text and project
...
/// each). The weights make "any strong signal, or two mediums" clear the confidence bar (score >= 2)
/// without a separate threshold check — the resolver just takes the unique top score. Two-plus candidates
/// tied at the top (including a 0-0 tie, i.e. no signals fired for either) throw rather than guess (KD4) —
/// never an alphabetical pick.
/// </remarks>
internal static class WebResourcesProjectResolver
{
    ...
    /// project is required), or when two-plus tie at the top score (R9 — never resolved by picking).
    /// </exception>
        ...
        string solutionFileName)
    {
        var candidates = projects
            ...
            .Where(File.Exists)
            .Where(path => !pluginProjectPaths.Contains(path))
        ...
        if (candidates.Count == 1)
            // Sole survivor of the plugin/PCF/test exclusions is the WebResources project by elimination
            // (KD3), even with zero signals below. Deliberate, not a gap: R5 says exactly one is expected,
            // so a lone non-plugin/PCF/test .csproj is it — nothing else in the solution file could be.
            return candidates[0];
        ...
        if (top.Count > 1)
            throw new FlowlineException(ExitCode.ConfigInvalid,
                $"'{solutionFileName}' has {candidates.Count} projects that could be the WebResources project " +
    ...
    // Cheap, name-first: a project file literally named "*test*" never needs its text read. The SDK-marker
    // fallback catches a differently-named test project (ClientHooksTests.csproj is caught by name already;
    // a project just called "Verification.csproj" referencing Microsoft.NET.Test.Sdk is not).
    static bool IsTestProject(string projectPath) =>
        Path.GetFileNameWithoutExtension(projectPath).Contains("test", StringComparison.OrdinalIgnoreCase) ||
    ...
    static IEnumerable<string> EnumerateSourceFiles(string dir, string[] extensions)
    {
        ...
        foreach (var subdir in EnumerateDirectoriesSafely(dir))
        {
            ...
            foreach (var file in EnumerateSourceFiles(subdir, extensions))
                yield return file;
        }
    }
