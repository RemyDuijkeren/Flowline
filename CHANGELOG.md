...
- **`flowline scaffold webresources` writes the WebResources project template on its own**: the template was reachable only as a side effect of `clone` or `init`, both of which connect to Dataverse to bring a whole solution into a repo. Anyone who wanted the template itself — working outside project mode, or adding web resources to a plugin-only repo migrated off spkl — was hand-copying files out of the Flowline repository. The command writes it locally instead, and reaches nothing: no Dataverse call, no authentication, no network, not even the update check, because a folder that is not a project yet should not need a PAC install or a git repo to get a build setup. It asks one question: where is the solution file? Found, the project is named after it and added to it, the same result `clone` produces. Not found, the template lands alone under a generic `WebResources.csproj`. Either way the project itself is written where you are standing, never moved somewhere you didn't point it. The search for the solution file walks *up*, because running this from `Plugins/` should join the repo you are in rather than leave a project no solution file knows about, and it is bounded by the two markers that mean project root: `.flowline`, and the `.git` of the repo you are working in. Neither is searched past, since a solution file above the repo root belongs to different work. Stand-alone use has neither marker, and that is not a gap to paper over: with nothing to bound the walk there is no walk at all, so a loose folder can never write itself into some unrelated solution file several levels up. `.flowline` is used as a marker on disk and nothing inside it is read. An earlier build branched on it to pick a project *file name*, and that name turns out to be cosmetic: the WebResources project compiles nothing and emits no assembly, and Flowline locates it by what it contains, never by what it is called. Whether the project reached a solution file is reported plainly, without `--verbose`, because that is the difference between a project `push` can find and a folder it cannot: either the project file and the solution file it was added to, or a warning naming what to do about it, including the `push --webresources` form that needs no solution file at all. `--output <path>` is simply "the folder I'm standing in": the project is written there and the search starts there. `--name <name>` names the project folder and its `.csproj` together, and addresses its own project: an unrelated `WebResources/` folder sitting there does not block it. A name ending in `Test` or `Tests` is refused, since Flowline reads a project named that way as a test project and would skip its web resources on every push. So is a second WebResources project in a solution file that already has one, because Flowline allows exactly one per project and the loser of that race is silently never pushed. Nothing is ever written over: a project already there is reported and skipped, and a folder holding template-named files that no project file claims — someone else's `package.json`, a half-finished experiment — is refused by name, because the template writer truncates rather than skipping and there is no flag to overrule it. Aliased `new`. A blocked write exits with the new code **20** (`WriteTargetOccupied`) rather than reusing 11 — nothing is missing or malformed there, something valid is in the way, and an agent branching on the exit code needs to tell those apart. One related fix on the older path: `clone` and `init` now recognise a stand-alone scaffold's generic `WebResources.csproj` and leave the folder alone with a warning, where before they saw no project, rewrote all eight templates, and destroyed whatever had been edited in them.

- **`drift` and `deploy` now say where each orphan came from**: detecting an orphan proves a component is in the environment and not in your source, but absence alone can't tell *removed on purpose* apart from *never yours*, and the operator was closing that gap from memory. Every orphan in the report now carries a verdict read from your repository's own history: removed from source — naming the commit, its author, its date, and its subject line as the stated reason — never in source, or unresolved when the lookup couldn't answer. Unresolved is deliberately never rendered or treated as never-in-source; a failed lookup shown as "never yours" would mark a real deletion safe to ignore. The commit subject is the point of the whole thing: dropping a workflow is reversible from source, while dropping a column destroys the values in it, so that call needs the reason behind the removal and not just the fact of one. Nothing about what Flowline deletes changed — no component became newly deletable, no handler changed tier, and no exit code moved. Web resources, security roles, copilots and custom APIs answer fully. Columns and connection references are declared inline in a shared file, so they answer removed-with-the-commit or unresolved, but never never-in-source — and only a removal from the table's own `Entity.xml` counts, since dropping a column from a form or view means it's used less, not that it's gone. Workflows, plugin assemblies and steps, custom API parameters, and table roots have no verified source mapping yet and read unresolved. A shallow or partial CI checkout can't see the history, so every verdict reads unresolved and no history query is issued at all rather than fetching every historical revision on demand. Nothing is cached between runs, so a failed lookup can never harden into an answer. `deploy` states up front how far its verdicts can be trusted: a build packed from this checkout compares exactly, a supplied `--path` artifact matches its solution version against your history and warns that a version match isn't proof, and a supplied artifact with no project source around it skips the lookup and says so.

- **Flowline now tells you when a newer version is published**: nothing in the `dotnet tool` install path announces a release, so people stayed on whatever they first installed and had no reason to look. The setup check that already greets you now asks NuGet — at most once a day, and only when you're at a terminal — then prints one line naming the version available, the one you're on, and the command that installs it. It's advice, not action: Flowline never touches its own install, because `dotnet tool` owns that and swapping the binary of something that writes to PROD would hide which code ran. Unattended runs skip the check entirely, no notice and no network call, since nobody there can act on it and an unexplained line derails an agent. The comparison is channel-matched, so a stable build is never nudged toward a prerelease and a prerelease build hears about the next one. A check that can't reach NuGet prints nothing, changes no exit code, and backs off for the day rather than paying the timeout on every command — `--verbose` says it was skipped, and `--no-cache` forces a fresh one.

- **`push` now warns which components still depend on a web resource before deleting or removing it**: before, a resource dropped from the solution or missing locally was deleted or unlinked with no visibility into what still pointed at it. A resource still referenced elsewhere — say by a form in another unmanaged solution — could be silently unlinked there too, and that only surfaced later, in an environment where the other solution wasn't present and a form still expected it. Push now asks Dataverse which components still depend on the resource and names them: the component-type label plus the record name where one exists (a system form or view by name), or the type label and object id where it doesn't (several ribbon component types have no nameable record). It's informational only — it never blocks the delete or removal and never changes the exit code; the operator decides whether the dependency is stale or expected. A resource a *managed* solution also holds closes with a note that it should ship downstream; a resource only another *unmanaged* solution holds closes with a note that that solution may not ship downstream — the case that can silently break a later environment. A failed lookup says the check couldn't run, rather than reporting no dependents. `--dry-run` shows the same warning before anything is written, and excludes forms the same run's form-event cleanup is about to unwire, so it doesn't warn about a link Flowline itself is removing. `--no-delete` skips the lookup entirely, since nothing is deleted or removed in that mode. If a delete still fails on Dataverse's own dependency check, the failure names the dependents already resolved during planning, no second lookup. Orphan cleanup (`deploy`'s pre-import pass and `drift`) reports the same dependents against whichever action it resolved, including for entries it only reports rather than executes. The check runs against the environment being pushed to, so it can't see a reference that exists only in a later one — it catches the ordinary case, not a guarantee.
...
- **`flowline sln add` finds the solution file from where you're standing**: it looked only in the exact folder it was run from, so the case it was built for — `flowline sln add Solution/X.cdsproj` from inside `Solution/`, with the solution file at the repo root — failed with "No .sln or .slnx in ..." and exit 3, even though the command's path-rewriting logic exists precisely to handle that layout. It now walks up, and the walk has the boundary the previous behavior was reaching for: it stops at the nearest `.flowline` or `.git` and never climbs past one, so the original hazard — running it somewhere loose and silently rewriting an unrelated solution file in a parent directory — stays closed. With neither marker anywhere there is nothing to bound the search, so no walk happens at all and only the folder you're in is examined. `scaffold` uses the same rule, from the same code, so both commands answer "where is my solution file" identically.
...
- **The orphan report now says who removes what on a managed deploy**: a managed solution already installed in the target imports as a Dataverse Upgrade, and that Upgrade removes every component the new version drops — Flowline itself deletes nothing on that path. The report said the opposite: entries read "would delete", and components Flowline can't remove through the SDK (tables, columns, roles) were split into a "can't be removed automatically" block that pointed at the Maker Portal — an instruction that isn't even possible against a managed solution, whose component list is read-only there. Every entry now reads "removed by the managed upgrade", the manual block is gone, and the summary says nothing needs removing by hand. The Prio1/2/3 tiers stay, because they still say which orphans are about to break the deploy. A closing note explains that components another solution owns only lose their membership — the component itself stays installed. Unmanaged deploys and `drift` are unchanged, and a first-time managed install still says cleanup waits for a later upgrade deploy.
...
- **`push --no-build` no longer pushes a stale plugin package and calls it a success**: NuGet's Pack step skips repacking when the package version didn't change — and with MinVer the version only moves on a commit, so editing source without committing recompiles the assembly and leaves the previous `.nupkg` sitting in `bin/Release`. Flowline clears that package before a build it runs itself, but `--no-build` runs neither the clear nor the pack, so the leftover package was what got pushed. Its content hash then matched the marker Dataverse already held, the push took the unchanged-content path, and every step and image registration reconciled against the *old* assembly's attributes — reporting "already up to date" and exiting `0` while nothing was registered. Push now compares the assembly inside the package against the one on disk, by content rather than by timestamp, and under `--no-build` stops with `ExitCode.ValidationFailed` naming the stale package and the two ways out: delete it and rebuild, or drop `--no-build`. A run that builds is unchanged — it still self-heals with one forced rebuild.

- **Orphan cleanup no longer looks hung while it works**: the spinner covered only the first of its Dataverse calls — the solution-component query — and tore down when that returned, leaving the identity resolution and per-handler lookups behind it (a dozen-plus further round trips, measured at ~8 seconds against a real solution) with nothing on screen at all. It now runs one spinner across the whole comparison, retitled per phase: querying components, resolving component ids, then checking the orphan candidates it found, with the candidate count in the label. Nothing about the report itself changed.

- **The resolved-profile line now names the environment it resolved for**: with no auth profile matching the target URL, profile resolution falls back to the tenant-wide UNIVERSAL profile — and the status line printed *that profile's* environment, the one it happened to be created against. So `deploy uat` announced "ROVM Dev" three times on a run whose target was ACC, reading as if the deploy were connecting to DEV. On a URL match the line is unchanged (the profile's own label carries the friendly name); on the fallback it now reads `— for <target url>`.

- **A managed deploy no longer fails right before the import**: `deploy` unpacks the zip it's about to import so the post-deploy checks read the real imported content, but `pac solution unpack` defaults to Unmanaged and rejects a managed package outright. Every managed deploy died there — after packing, the missing-component check, the solution checker, and the backup had all run — with "Unpack failed ... is this a valid solution zip?" against a zip that was perfectly valid. The unpack now matches the solution's managed setting.

- **A Dataverse request timeout now reports itself instead of dumping a stack trace**: a slow environment — a big plugin package upload is the usual one — left the SDK's request timeout unhandled, so the run ended in a wall of internal frames and a generic exit 1. It now exits `16` (Timeout) with a one-line report. The wording matters: the timeout is client-side, so the write may well have landed, and the message says so and points at a re-run rather than claiming the push failed. Both shapes the SDK produces are caught — the SOAP channel's `TimeoutException` and the HttpClient path's cancelled task — and the latter is told apart from a real Ctrl+C, which still exits `130` as before.
