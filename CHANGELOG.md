...
- **New `settings` command reads and changes per-environment configuration**: `flowline settings` is a branch whose operations are the things it does. `settings push <target>` reads a settings file and reconciles the environment to it — environment variable values, connection reference bindings, and the on/off state of cloud flows, classic workflows and plugin steps. `settings pull [target]` captures a file from a live environment instead, merging into an existing one rather than overwriting it: values already in the file are kept, components that appeared are added, and an entry whose component is gone is reported rather than dropped. Omit the environment and it captures every environment the project configures, one role-named file each, reporting each on its own line and exiting 18 if one couldn't be reached. Against a URL it names the file from the role it infers from the environment, and refuses to fall back to the shared file when it can't infer one. Both operations name the settings file with `--settings-file`, which `push` reads and `pull` writes; `pull` also takes `--from <zip-or-folder>` for the solution source.
