...
- **New `configure` command applies per-environment settings**: `flowline configure <target>` reads a settings file and reconciles the environment to it — environment variable values, connection reference bindings, and the on/off state of cloud flows, classic workflows and plugin steps. Cloud flows and classic workflows get a section each (`CloudFlows` and `Workflows`), and both are name-to-`true`/`false` maps rather than arrays of objects, because this is the half of the file people edit by hand. Business process flows, business rules, dialogs and desktop flows share the same Dataverse table and are left alone entirely: never read, never written, never pulled into the file. Only components the file names are touched; everything else is left alone and reported. Re-running the same file changes nothing, so it is safe on every pipeline run, and `--dry-run` reports exactly what would change without writing. `--pull [zip-or-folder]` captures a file from a live environment instead, merging into an existing one rather than overwriting it: values already in the file are kept, components that appeared are added, and an entry whose component is gone is reported rather than dropped. Secrets are never read — a Key Vault-backed variable stores a reference, which is pulled, and every other secret store gets a placeholder and a warning. A flow Dataverse has suspended is treated as off but never as already-off: the file decides its state in both directions, so one declared off is returned to Draft and one declared on is started again, and either way the run says the flow had been suspended. Works inside a project and standalone (`--solution-name`, or `--pull <zip>`). Exit 18 when some components failed, 19 when the file matched nothing in the target — usually the wrong file or the wrong environment — and 130 when a run was interrupted, which still prints the components it applied before stopping so an operator knows what landed.
