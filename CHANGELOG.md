...
- **`drift` no longer exits non-zero by default when it finds orphans**: it exits `0` after a completed comparison, like it always has for a clean one; pass `--exit-code` to get `22` (`ChangesFound`, shared with `diff`) when there's something to report, matching `git diff --exit-code`. Previously exited `15` (`ValidationFailed`) whenever orphans were found.
- **`clone` fails instead of guessing when several environments are configured and `--env` is omitted**: non-interactively it used to clone from the first configured match silently; it now exits `15` (`ValidationFailed`) naming `--env`. Interactively it still prompts.
- **`configure` is now `settings`**: `flowline configure <target>` is `flowline settings push <target>`, and `flowline configure <target> --pull` is `flowline settings pull <target>`. The `--pull [zip-or-folder]` flag is gone: the artifact is `--from` on the capture operation, and `--settings-file` names the settings file on both. `configure` never appeared in a release, so there is no alias and no deprecation — this note is for anyone tracking the unreleased branch.
- **Unknown options are now a parse error**: `flowline <command> --bogus` exits `15` (`ValidationFailed`) with `Error: Unknown option 'bogus'.` instead of being accepted or silently ignored.

### Added

- **`flowline diff` reports what changed between two points in git history**: it names the solution components behind a diff instead of the XML files, using the same change tree `sync` prints. With no options it compares `HEAD` against the working tree, counting untracked files as additions; `diff A` moves the left side, and `diff A B` compares two refs without consulting the working tree. It contacts no environment: no connection, no authentication, no network. `--write` opts into a `CHANGES.md` at the repo root, or at a path you name, and `--exit-code` returns the new `ChangesFound` (22) when there was anything to report, so a CI job can branch on "is there anything to sync or deploy" without parsing output. 22 is not a failure. `diff` is the git-history counterpart to `drift`, which compares committed source against a live environment.
- **New `settings` command reads and changes per-environment configuration**: `flowline settings` is a branch whose operations are the things it does. `settings push <target>` reads a settings file and reconciles the environment to it — environment variable values, connection reference bindings, and the on/off state of cloud flows, classic workflows and plugin steps. `settings pull [target]` captures a file from a live environment instead, merging into an existing one rather than overwriting it: values already in the file are kept, components that appeared are added, and an entry whose component is gone is reported rather than dropped. After writing the file, a pull now offers to fill in what it could not capture: an environment variable with no value anywhere, and a connection reference with nothing bound, the latter through the same connection picker `settings connref` uses. It asks once per file and nothing at all when nothing is missing, skips the `<set-this-secret>` placeholder so a secret is never typed into a committed file, and saves the capture before asking so an abandoned session never costs it. Non-interactively it reports the count and exits `0`. Omitting the environment used to sweep every configured one unattended; it now fails with exit 15 naming the argument, the same as `push`, and at a terminal offers a multi-select of the configured roles with all of them pre-selected, so one Enter still captures the lot. It writes one role-named file per environment, reports each on its own line, and exits 18 if one couldn't be reached. Against a URL it names the file from the role it infers from the environment, and refuses to fall back to the shared file when it can't infer one. Both operations name the settings file with `--settings-file`, which `push` reads and `pull` writes; `pull` also takes `--from <zip-or-folder>` for the solution source.

  Two more operations change one component without a file at all: `settings state` and `settings value`, each taking the environment then the component name, named for what they do rather than for what they address. `--type` narrows them — on `state`: `flow`, `workflow`, `rule`, `bpf`, `action`, `plugin`; on `value`: `envvar`, `connref` — and a name with no `--type` is matched across every class the command covers, stopping and naming both when two match. `state` reaches beyond cloud flows and classic workflows to business rules, business process flows and actions: all of them are rows in the same process table with the same on and off. A settings file has no section for those three, so a `pull` never writes them and a `push` never touches them, which means one turned off here stays off. Because the kind is a filter rather than a command, a process category Dataverse adds later becomes another `--type` value instead of another command. Pass `--on` or `--off` for a state, or `--value` for a value, and it writes; pass neither and it reads and prints what's there. Naming the environment before the operation, `flowline settings dev flow`, used to answer `Unknown command 'dev'`. It now says which token is the operation and prints the line to run, carrying the rest of the invocation and its quoting into the suggestion; with no operation at all it names the seven. A mistyped operation is still left to the parser. The picker gathers components whose names share leading text under a heading, so a plugin step's namespace or a flow's entity prefix is written once instead of on every row. The shared text is found by cutting names at `.`, `]`, `|` and `:` rather than configured, a heading is never half a word, and a group only forms when it saves more characters than the line its heading costs — so nine plugin steps sharing a namespace group and two flows sharing `[SalesOrderDetail]` do not. Headings cannot be picked; only the components under them can. Blocks are ordered by their most interesting member, with an ungrouped component winning a tie, because a group is one contiguous block and cannot be split across the suspended-then-off-then-on tiers. Under `--type` the type column is dropped, since it would say the same word on every row. **Esc** cancels any picker and exits `130`, on the component list, the state prompt, the connection picker and the environment picker. Text and yes/no prompts keep a blank answer and `n` as their way out, since Spectre carries no cancel result on those. The prompt that follows a read now offers its choices with the same shape and colour the list uses (`● on`, `○ off`) rather than as bare words, and quotes the component name in its title, so a rule called `Set reisduur` no longer produces `Set Set reisduur to:`. In a state list each line leads with the component's state, as a coloured shape and a word (`● on`, `◐ suspended`, `○ off`), ordered suspended then off then on so the ones worth acting on are at the top. It used to trail the name, which put it off the end of the line for a plugin step named after its class and message. The word is part of the picker's search text, so typing `off` filters to the ones that are. On `connref`, the interactive prompt lists the environment's connections for that reference's connector and you pick one, rather than typing a generated id copied out of the maker portal; the menu also carries "enter an id by hand" (a connection someone else owns is not in your list) and "create a new connection in the maker portal", which opens that environment's page and offers to list again. Omit the name and you get a searchable picker; pick one and it asks for the new state or value, as a read at a terminal does too. In a non-interactive run, omitting the name lists what's there and exits 0 — with each one's state for the three state kinds, names only for `envvar` and `connref`, so listing a solution never prints every variable's value at once. Turning a flow on at go-live no longer means opening the maker portal. A change made this way is deliberate drift, so when the environment's settings file would put the component back, the run says so and names the file.

  Cloud flows and classic workflows get a section each in the file (`CloudFlows` and `Workflows`), both name-to-`true`/`false` maps rather than arrays of objects, because this is the half of the file people edit by hand. Business process flows, business rules, dialogs and desktop flows share the same Dataverse table and are left alone entirely: never read, never written, never captured. Only components the file names are touched. Re-running the same file changes nothing, so it is safe on every pipeline run, and `--dry-run` reports exactly what would change without writing. Secrets are never read on the capture path — a Key Vault-backed variable stores a reference, which is captured, and every other secret store gets a placeholder and a warning. A flow Dataverse has suspended is treated as off but never as already-off: the file decides its state in both directions, so one declared off is returned to Draft and one declared on is started again, and either way the run says the flow had been suspended. Works inside a project and standalone (`--solution-name`, or `--from <zip>`). Exit 18 when some components failed, 19 when the file matched nothing in the target — usually the wrong file or the wrong environment — and 130 when a run was interrupted, which still prints the components it applied before stopping so an operator knows what landed.
- **`clone` and `pull` keep the shared settings template in step with the solution**: the un-suffixed `deploymentSettings.json` is regenerated from the unpacked solution source, so a solution that gains an environment variable or a connection reference gains the key too, with an empty value. Values already filled in survive, and a key whose component left the solution is reported rather than deleted. It reads the solution on disk only — no environment, no authentication, no network — and it writes no values, because an empty value is skipped on apply and that is what makes a file every environment can fall back to safe to generate automatically. Component state is deliberately never written there: a flow's state has no inert form, so listing one at all would declare it.
...
- **The export command is now called `pull`**: `sync` stays as a permanent alias, so existing scripts keep working.
- **`scaffold plugins`** joins `scaffold webresources`: writes a `Plugins` project template into the current folder the same way, and is added to the nearest solution file when one is found.
- **A URL passed to `--env` that isn't in `.flowline` yet gets offered a role to save under**: a `Production` environment is `prod`; otherwise the role is read from a keyword in the URL's host name (`dev`, `develop`, `development`; `test`, `tst`, `testing`, `qa`, `sit`; `uat`, `acc`, `acceptance`, `acceptatie`, `preprod`, `staging`, `stage`, `stg`), matched as a whole hyphen-separated part in any position, digits ignored, last one wins; then from the same keywords in the environment's display name; then a `Developer` environment is `dev`. A Sandbox with no keyword in its name isn't inferred. Interactively it's a picker, pre-selected on the inferred role, with an escape hatch to use the URL once without saving. Non-interactively it saves the inferred role and prints `Saved to .flowline: <key> (inferred from ...)`, or fails naming the key to add (`DevUrl`, `TestUrl`, `UatUrl`) when nothing could be inferred.
...
- **`--env` is optional in standalone `generate`**: with no URL passed, `generate` targets the environment of the currently active resource-specific PAC auth profile, matching what standalone `push` already did. A universal profile carries no environment URL, so it still can't stand in for one, and an explicit `--env` always wins.

- **A reachable active PAC auth profile is used without a switch prompt**: when the resolved target has a URL match that isn't PAC's active profile, Flowline now probes whatever profile *is* active against the target first; if it can reach it, that profile is used outright, no switch, no prompt. The existing URL match, ambiguity picker, switch prompt (default no, `-a` to consent, no switch back) still apply when the active profile can't reach the target.
- **`provision --copy` defaults to `minimal` for `dev` and `full` for `test`/`uat`**, applying to every role rather than one fixed default; pass `--copy full|minimal` to override.
- **A declined `deploy` first-import confirmation now exits `17` (`ForceRequired`)** instead of `130` (`Cancelled`); re-run with `--force first-import`.
- **Every flag that persists to `.flowline` documents it in its own `--help` text**, ending the description with "(saved to .flowline)".
- **`flowline --help`'s examples and `deploy`'s target list were updated for the current command surface**: the target list no longer names `dev` (rejected outright by `deploy`).
...
- **`flowline scaffold webresources`** (aliased `new`): writes the WebResources project template locally, with no Dataverse call, no authentication and no network. It finds the solution file by walking up to the nearest `.flowline` or `.git` and adds the project to it; without one, the template lands alone under a generic `WebResources.csproj`. `--output <path>` sets where it is written, `--name <name>` names the project folder and its `.csproj`. Nothing is ever overwritten: a blocked write exits with the new code **20** (`WriteTargetOccupied`). `clone` and `init` now leave a stand-alone scaffold's folder alone instead of rewriting its templates.

- **`drift` and `deploy` say where each orphan came from**: every orphan carries a verdict read from your git history, either removed from source (naming the commit, author, date and subject line), never in source, or unresolved when the lookup could not answer. Nothing about what Flowline deletes changed.

- **Flowline tells you when a newer version is published**: interactive runs ask NuGet at most once a day and print the available version, the one you are on, and the install command. Unattended runs skip the check and make no network call. Flowline never updates itself.

- **`push` warns which components still depend on a web resource** before deleting or unlinking it, naming each dependent by record name or object id. Informational only: it never blocks the operation or changes the exit code.

- **`deploy` checks the target for missing components before importing**: Dataverse stops at the first unresolvable dependency, so the new gate reports all of them in one pass, with the solution that owns each, before anything is written. Skip it with `--skip-component-check`.

### Changed

- **`flowline sln add` finds the solution file from where you are standing**: it now walks up, bounded by the nearest `.flowline` or `.git`. `scaffold` uses the same rule from the same code.

- **`push` no longer overwrites a web resource another solution owns** (breaking): it refuses and names the file, the logical name and the owning solution, reporting every offending file in one pass. A resource belonging to no other solution is still adopted.

- **`// flowline:depends` also means "I don't own this"**: a foreign-owned resource declared as a dependency is skipped rather than blocking the push, and the skip warns to remove the now-unused local file.

- **The orphan report says who removes what on a managed deploy**: entries read "removed by the managed upgrade" instead of "would delete", since a Dataverse Upgrade removes the dropped components and Flowline deletes nothing on that path. Unmanaged deploys and `drift` are unchanged.

- **Verbose web resource snapshot trees print side by side**, Local on the left and Dataverse on the right.

### Removed

- **`push --scope formevents`** (breaking): use `--scope webresources`, which already ran form event registration.

### Fixed

- **`push --no-build` no longer pushes a stale plugin package and calls it a success**: it compares the assembly inside the package against the one on disk and stops with `ValidationFailed`, naming the two ways out.

- **`push` no longer re-uploads an unchanged plugin package**: change detection hashed the container NuGet rewrites on every pack, so it now hashes the `lib/` payload Dataverse consumes. The first push after upgrading re-uploads once, then settles.

- **A managed deploy no longer fails right before the import**: the pre-import unpack now matches the solution's managed setting instead of defaulting to Unmanaged.

- **A Dataverse request timeout now exits `16` with a one-line report** instead of dumping a stack trace. The timeout is client-side, so the message says the write may have landed.

- **Orphan cleanup no longer looks hung while it works**: one spinner now covers the whole comparison, retitled per phase.

- **The resolved-profile line names the environment it resolved for** when profile resolution falls back to the tenant-wide UNIVERSAL profile, instead of printing that profile's own environment.

- **The web resource plan line counts solution adds and removes**, and says "no changes" when there are none.

- **`push --no-publish` warns whenever nothing publishes**, not only when web resources are out of scope.

- **`[PreImage]` on a `CreateMultiple` step is now rejected**, matching the existing rule for `Create`, and the "entity images are not supported" message is derived from the real allowlist instead of a drifted hardcoded list.

- **Verbose logging no longer prints markup as literal tags.**
...
- **Removing a plugin project from a `.nupkg` package broke the push**: dropping an assembly from a plugin package failed with `Unable to delete '<type>' plugintype due to 1 step(s) registered on it`, after the package content write had already committed. Dataverse rejects an update that removes an assembly whose plugin types still carry step registrations, and expects those registrations to be cleared first — which `push` already did for a class removed from a surviving assembly, but not for an assembly that disappeared from the package entirely. It now clears the dropped assembly's images, Custom API parameters and properties, steps and Custom APIs before the content update, leaving the plugin types and the assembly record to the update itself. `--dry-run` previews each assembly that would be added or dropped; `--no-delete` skips the clearing and warns that the update will be rejected rather than deleting behind the flag's back.

- **`push --force delete-orphans` deleted every Custom API under the publisher prefix**: clearing a single orphan plugin assembly deleted *all* Custom APIs (and their request parameters and response properties) whose unique name starts with the solution's publisher prefix — other projects', other repos', regardless of what the orphan actually implemented — and did it silently, since the cascade preview only listed plugin types, steps and images. A snapshot's plugin types, steps and images are scoped to one assembly, but its Custom APIs are resolved prefix-wide; the orphan path deleted the whole list. It now deletes only the Custom APIs bound to that orphan assembly's own plugin types, and the cascade preview lists them like any other child, so deleting a public API surface is never invisible.

- **Orphan detection on the plugin-package path**: a solution whose plugin projects all pack to a `.nupkg` never got an orphan pass at all — the check ran only on the classic assembly path, and the fallback ("deploy's orphan cleanup covers it") does not apply to a development environment, since `deploy` targets a downstream environment and `deploy dev` is rejected outright. Such a solution had no way to see or clear its orphans. `push` now runs the same orphan-assembly and orphan-step passes on the package path. The original reason for skipping them — a multi-assembly package reading its own secondary assemblies as orphans — no longer applies now that the exclusion takes the whole pushed set; the package path additionally unions in its own reflected assembly names, so the guarantee does not depend on the caller.
